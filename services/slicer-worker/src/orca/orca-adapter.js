...
export class OrcaSlicerAdapter {
  ...
  async slice({
    ...
    if (extension === "3mf") {
      ...
      /*
       * STL/OBJ sempre passam pelo
       * pré-processamento.
       *
       * Mesmo em mm (unitScale === 1),
       * precisamos remover a translação
       * arbitrária gravada no arquivo.
       *
       * Isto NÃO altera:
       * - orientação;
       * - proporção;
    ...
    if (execution.code !== 0) {
      ...
      /*
       * Mantém detalhes somente
       * para observabilidade interna.
       *
       * Eles não são enviados ao cliente.
       */
      error.details =
        safeFailureDetails(
          execution
...
async function findOutputArchive(
  outputDir,
  requestedOutput
) {
  try {
...
export function classifyExecutionFailure(
   ...
   * 256 - 50 = 206
   *
   * Para STL/OBJ, após normalização
   * geométrica, -50 significa que o
   * objeto continua sem caber na área
   * imprimível.
   *
   * Não aplicamos automaticamente
   * essa interpretação ao 3MF,
   * porque uma plate vazia também
   * pode produzir -50.
   */
